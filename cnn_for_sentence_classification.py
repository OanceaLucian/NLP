...
with open("wiki_sentences.txt","r") as f:
    wikipedia = f.readlines()

with open("humorous_oneliners.txt","r") as f:
...
if train == 1:
    ...
    model.summary()
    filepath = "weights-improvement_50_-{epoch:02d}-{val_acc:.2f}.hdf5"
    checkpoint = ModelCheckpoint(filepath, monitor='val_acc', verbose=1, save_best_only=True, mode='max')
    earlyStopp = EarlyStopping(monitor='val_acc', patience=4, verbose=0, mode='auto')
    callbacks_list = [checkpoint, earlyStopp]
    model.fit(x_train, y_train, validation_data=(x_val, y_val),
          nb_epoch=50, batch_size=128, verbose = 1,callbacks=callbacks_list)
predict = 1
if predict:
    model.load_weights("weights-improvement_50_-08-0.95.hdf5")
    model.compile(loss='categorical_crossentropy',
                  optimizer='rmsprop',
                  metrics=['acc'])
